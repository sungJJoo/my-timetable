...
function readAll() {
  var ev = sheet("events", EV_H).getDataRange().getValues().slice(1)
    ...
    .map(function (r) {
      return { id: String(r[0]), day: +r[1], start: +r[2], end: +r[3], title: String(r[4]), place: String(r[5]), color: +r[6], author: String(r[7] || ""), until: toDateStr(r[8]) };
    });
  var nt = sheet("notes", NT_H).getDataRange().getValues().slice(1)
    .filter(function (r) { return r[0] !== ""; })
    .map(function (r) {
      return { id: String(r[0]), text: String(r[1]), ts: +r[2], author: String(r[3] || "") };
    });
